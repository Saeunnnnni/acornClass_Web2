...
<body>
	<div class="container">
		...
		<h1>El로 page scope에 저장된 값 추출</h1>
		<p>너 이름은 <strong>${requestScope.yourName }</strong></p>
				 <!--requestScope생략가능  -->
		<p>너 이름은 <strong>${yourName }</strong></p>
		
		<%--위의 EL은 아래의 코드를 대신할수 있다. --%>
		...
		<p>학원 이름은 <strong>${applicationScope.companyName }</strong></p>
				 <!--applicationScope생략가능  -->
		<p>학원 이름은 <strong>${comanyName }</strong></p>
