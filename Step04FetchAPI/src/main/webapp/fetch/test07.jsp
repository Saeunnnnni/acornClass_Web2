...
<body>
	...
	<script>
		document.querySelector("#myForm2").addEventListener("submit",(e)=>{
			e.preventDefault();
			//gura_util.js 안에 ajaxFormPromise() 함수 활용하기 
		...
		*/
		document.querySelector("#myForm").addEventListener("submit", (e)=>{
			//이벤트 객체 안에 있는 preventDefault() 함수를 호출하면 폼 제출이 막아짐
			e.preventDefault();
			//fetch 함수를 이요해서 페이지 전환 없이 입력한 내용을 읽어와서 직접 제출하기
			...
			fetch("login.jsp",{
		          ...
		          body:queryString
		          //fetch함수를 사용할때 object로 옵션을 전달해야함 / 문자열을 querString으로 만들어서 전달해야함 
			})
			.then(res=>res.json())
			.then((data)=>{
				console.log(data)
